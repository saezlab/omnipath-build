...
PROTEIN_REFERENCE_OUTPUT_FILENAME = 'protein_identifier_lookup.parquet'
UNIPROT_SECONDARY_OUTPUT_FILENAME = 'protein_identifier_lookup.parquet'
...
def _protein_identifier_rows(
    ...
    for row in uniprot_resource.reference_id_translation.raw(taxonomy_ids=taxonomy_ids):
        ...
        yield {
            ...
            'mapping_type': 'uniprot_primary' if key_type == UNIPROT_TYPE else 'uniprot_reference',
        }
...
def build_protein_identifier_lookup(
    ...
    """Build one long protein resolver table directly from UniProt translation rows."""
    activate_raw_download_data_dir()
...
def materialize_proteins(
    ...
    activate_raw_download_data_dir()
    protein_identifier_rows = write_parquet_from_dict_rows(
    ...
    return {
        'protein_identifier_lookup_rows': protein_identifier_rows,
    }
