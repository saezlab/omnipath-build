...
4. **Source provenance is also compact `label:id` strings** (e.g. `FooDB:1766`), not split into name/id arrays.
5. **Filterable annotation terms (no value/unit)** are materialized into one flattened array for Meilisearch.
6. **Derived semantic fields** (direction/sign/activation) are optional and can be computed downstream.
...
- `evidence: [ ... ]`
  ...
  - `sources: string[]` (`label:id`, e.g. `FooDB:1766`)
...
- `evidence: [ ... ]`
  ...
  - `sources: string[]` (`label:id`, e.g. `BindingDB:1766`)
...
1. sources (sorted `label:id` strings)
2. interaction_entity_ids (if present)
3. normalized annotation tuples `(term, value, unit)`

Then assign 1..n.

## Why no `member_instance_ids` in v2 docs

Entity instances are an internal normalization artifact (entity + source + annotations). For search payloads, we can treat each evidence item as a canonicalized bundle of:

- sources
- annotations
- interaction/entity context

So we can omit raw `member_instance_ids` from the public search schema and still preserve the information needed by downstream users.
