# Direct-to-Postgres Plan

## Scope

Build a separate prototype under `omnipath_build/` which streams `inputs_v2` rows
...
1. Define omnipath_build PostgreSQL DDL for refresh-loaded source evidence:
...
```bash
uv run python -m omnipath_build.cli canonicalize --source intact --dataset interactions
```

Resolver mappings are materialized by `omnipath_build.resolver`:

```bash
uv run python -m omnipath_build.cli build-resolver uniprot chebi hmdb lipidmaps swisslipids pubchem
uv run python -m omnipath_build.cli load-resolver
```
...
into the omnipath_build chemical resolver parquet. For development, pass
...
For scratch loads, pass `DROP_EXISTING=1`; schema setup defers
...
Omit `SOURCE`/`SOURCES` to discover and run every raw-backed omnipath_build source:
...
omnipath_build evidence tables. `canonicalize` resolves scoped evidence into entity
...
1. `omnipath_build/db/schema.py` for DDL.
2. `omnipath_build/ingest/` for direct streaming ingest.
3. `omnipath_build/cli.py` with `init-db`, `load-resolver`, `ingest`,
