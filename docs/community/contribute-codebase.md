...
dependencies needed for running omnipath_build (`[tool.poetry.dependencies]` with the
...
developing omnipath_build using test-driven development. Please make sure that you
...
If you are brand new to omnipath_build or open-source development, we recommend
...
omnipath_build is hosted on GitHub, and to contribute, you will need to sign up for a
...
### Create a fork of omnipath_build

You will need your own fork of omnipath_buildin order to eventually open a Pull
Request. Go to the omnipath_build project page and hit the Fork button. Please
...
```bash
git clone https://github.com/your-user-name/omnipath_build.git
cd omnipath_build
git remote add upstream https://github.com/omnipath_build/omnipath_build.git
git fetch upstream
```

This creates the directory `omnipath_build` and connects your repository to the
upstream (main project) *omnipath_build* repository. They have the same name, but
your local repository and fork are separate from the upstream repository.

### Creating a feature branch

Your local `main` branch should always reflect the current state of omnipath_build
repository. First ensure it's up-to-date with the main omnipath_build repository.
...
to create a branch called `my-new-feature-for-omnipath_build`

```bash
git checkout -b my-new-feature-for-omnipath_build
```

This changes your working branch from `main` to the
`my-new-feature-for-omnipath_build` branch. Keep any changes in this branch specific
to one bug or feature so it is clear what the branch brings to *omnipath_build*. You
...
When making changes, follow these omnipath_build-specific guidelines:
...
Now your code is on GitHub, but it is not yet a part of the omnipath_build project.
...
1. Write a descriptive title that includes prefixes. omnipath_build uses a convention for title prefixes, most commonly, `feat:` for features, `fix:` for bug fixes, and `refactor:` for refactoring.
...
branch with changes in the omnipath_build `main` branch, run:

```shell

    git checkout my-new-feature-for-omnipath_build
    git fetch upstream
...
```shell
    git push origin my-new-feature-for-omnipath_build
```
