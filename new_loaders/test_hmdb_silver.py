#!/usr/bin/env python3
"""Test HMDB bronze → silver transformation."""
...
def main():
    """Test HMDB processing."""
    logger.info("=" * 60)
    logger.info("Testing HMDB bronze → silver transformation")
    logger.info("=" * 60)
    ...
    with SourceProcessor(
        ...
        for function_name, parquet_file in results.items():
            ...
            logger.info(f"  Sample identifier: {sample[1] if len(sample) > 1 else 'N/A'}")
            logger.info(f"  Sample name: {sample[4][:50] if len(sample) > 4 and sample[4] else 'N/A'}...")
