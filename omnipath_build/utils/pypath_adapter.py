...
class PyPathAdapter:
    ...
    def _discover_methods(self) -> None:
        ...
        for importer, modname, ispkg in pkgutil.iter_modules(pypath.inputs.__path__):
            try:
                if ispkg:
                    ...
                    try:
                        ...
                        actual_module = __import__(f'pypath.inputs.{modname}', fromlist=[''])
                        ...
                        for attr_name in dir(actual_module):
                            ...
                            if hasattr(attr, '__module__') and f'pypath.inputs.{modname}' in attr.__module__:
                                ...
                                try:
                                    ...
                                    if self._returns_namedtuple(method_info):
                                        ...
                                        method_count += 1
                                        self.logger.debug(f"Found package method: {full_name}")
                                    else:
                                        self.logger.debug(f"Skipped {full_name} - doesn't return namedtuple")
                                    
                                except (ValueError, TypeError) as e:
                                    self.logger.debug(f"Could not process method {full_name}: {e}")
                        
                    except Exception as e:
                        self.logger.debug(f"Could not process package {modname}: {e}")
                    ...
                    for attr_name in dir(actual_module):
                        ...
                        if not hasattr(attr, '__module__') or not attr.__module__.endswith(modname):
                            continue
                        ...
                        try:
                            ...
                            if self._returns_namedtuple(method_info):
                                self._method_cache[full_name] = method_info
                                method_count += 1
                                self.logger.debug(f"Found regular method: {full_name}")
                            else:
                                self.logger.debug(f"Skipped {full_name} - doesn't return namedtuple")
    ...
    def get_available_methods(self, filter_by_module: Optional[str] = None) -> List[PyPathMethodInfo]:
        """
    ...
    def get_method_info(self, method_name: str) -> Optional[PyPathMethodInfo]:
        """
    ...
    def get_resource_data(self, method_name: str, **kwargs) -> pd.DataFrame:
        """
        ...
        try:
            ...
            raise BronzeLoaderError(f"PyPath method execution failed: {e}") from e
    
    def _convert_to_dataframe(self, data: Any, method_name: str) -> pd.DataFrame:
        """
        ...
        if isinstance(data, pd.DataFrame):
            ...
            try:
                ...
                self.logger.warning(f"Could not convert iterable to DataFrame: {e}")
    ...
    def _clean_dataframe_for_parquet(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Clean DataFrame by converting complex objects to string representations
        that can be saved to parquet format.
        ...
        for column in df_cleaned.columns:
            try:
                ...
                try:
                    ...
                    self.logger.debug(f"Converting column '{column}' to string due to complex objects")
                ...
                sample_val = df_cleaned[column].iloc[0] if len(df_cleaned) > 0 else None
                if sample_val is not None and hasattr(sample_val, '__class__'):
                    ...
                    if val_type not in ['str', 'int', 'float', 'bool', 'NoneType']:
                        self.logger.debug(f"Converting column '{column}' (type: {val_type}) to string")
                        df_cleaned[column] = df_cleaned[column].astype(str)
            except Exception as e:
                # Fallback: convert problematic columns to string
                self.logger.warning(f"Error processing column '{column}': {e}. Converting to string.")
                df_cleaned[column] = df_cleaned[column].astype(str)
        
        return df_cleaned
    
    def save_to_parquet(self, method_name: str, output_path: Path, 
                       resource_id: str, dataset_name: str, **kwargs) -> Tuple[Path, int]:
        """
    ...
    def save_to_temp_csv(self, method_name: str, resource_id: str, 
                        dataset_name: str, **kwargs) -> Tuple[Path, int]:
        """
        ...
        row_count = len(df)
        self.logger.info(f"Saved {row_count} rows to temporary CSV: {temp_path}")
    ...
    def list_available_modules(self) -> List[str]:
        ...
        """
        modules = set(method_info.module_name for method_info in self._method_cache.values())
    ...
    def search_methods(self, query: str) -> List[PyPathMethodInfo]:
        ...
        for method_info in self._method_cache.values():
            ...
            # Check docstring
            if method_info.docstring and query_lower in method_info.docstring.lower():
                matches.append(method_info)
    ...
    def extract_namedtuple_from_annotation(self, method_info: PyPathMethodInfo) -> Dict[str, List[str]]:
        """
        ...
        try:
            ...
            if hasattr(ret_annotation, '__args__') and ret_annotation.__args__:
                ...
                if hasattr(inner_type, '_fields'):
                    ...
                    self.logger.debug(f"Extracted namedtuple {type_name} with fields {fields} from annotation")
            ...
            self.logger.debug(f"Could not extract namedtuple from annotation for {method_info.full_name}: {e}")
            return {}
    ...
    def extract_source_fields(self, method_name: str) -> Dict[str, List[str]]:
        ...
        try:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.Assign):
                    ...
                    if isinstance(node.value, ast.Call):
                        ...
                        func_name = None
                        if hasattr(node.value.func, 'attr') and node.value.func.attr == 'namedtuple':
                            func_name = 'namedtuple'
                        elif hasattr(node.value.func, 'id') and node.value.func.id == 'namedtuple':
                            func_name = 'namedtuple'
            ...
            if namedtuples:
                self.logger.debug(f"Extracted {len(namedtuples)} namedtuple definitions from {method_name}")
            
            return namedtuples
            
        except Exception as e:
            self.logger.debug(f"Could not extract source fields from {method_name}: {e}")
            return {}
    ...
    def print_method_help(self, method_name: str) -> None:
        ...
        if params:
            ...
            for param_name, param_info in params.items():
                default_str = f" = {param_info['default']}" if param_info['default'] is not None else ""
