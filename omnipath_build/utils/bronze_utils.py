...
class BronzeWriter:
    ...
    def __init__(self, pg_config: Dict[str, str], duckdb_conn: duckdb.DuckDBPyConnection):
        """
        ...
        try:
            with self._get_pg_connection() as conn:
                with conn.cursor() as cur:
                    ...
                    if source_type == 'csv':
                        cols, rows = self._read_csv_sample(data_source, delimiter, has_header)
                    ...
                    if not cols or not rows:
                        logger.warning(f"No data found for bronze table {table_name}")
                        return 0
                    ...
                    conn.commit()
                    logger.info(f"✅ Wrote {row_count} rows to PostgreSQL bronze.{table_name}")
                    return row_count
                    
        except Exception as e:
            logger.error(f"Failed to write to PostgreSQL bronze.{table_name}: {e}")
    ...
    def _read_csv_sample(self, csv_path: Path, delimiter: str, 
                        has_header: bool) -> Tuple[List[str], List[Tuple]]:
        """Read sample data from CSV file."""
        temp_table = f"temp_bronze_csv_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        ...
        try:
            ...
            # Get columns
            cols = [row[0] for row in self.duckdb_conn.execute(f"""
                SELECT column_name
            ...
            # Get data
            rows = self.duckdb_conn.execute(f"SELECT * FROM {temp_table}").fetchall()
    ...
    def _read_parquet_sample(self, parquet_path: Path) -> Tuple[List[str], List[Tuple]]:
        """Read sample data from Parquet file."""
        temp_table = f"temp_bronze_parquet_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        ...
        try:
            ...
            if not cols:
                logger.warning(f"No data columns found in {parquet_path}, only metadata columns exist")
                return [], []
    ...
    def _create_bronze_table(self, cursor: psycopg2.extensions.cursor, 
                           table_name: str, columns: List[str]) -> None:
    ...
    def _insert_bronze_data(self, cursor: psycopg2.extensions.cursor,
                          table_name: str, columns: List[str],
                          rows: List[Tuple], resource_id: str,
