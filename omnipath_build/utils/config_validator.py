...
class PyPathConfigValidator:
    ...
    def __init__(
        ...
        self.logger.debug('Config validator initialized')
    ...
    def validate_database_module(
        self, database_name: str, module_name: str
    ) -> tuple[bool, list[str]]:
        ...
        try:
            ...
            if extends_path:
                template_errors = self._validate_against_template(
                    config, extends_path, module_name
                )
        ...
        if is_valid:
            self.logger.debug(
                f'✓ {database_name}/{module_name} validation passed'
            )
        else:
            self.logger.warning(
                f'✗ {database_name}/{module_name} validation failed with {len(errors)} errors'
            )
        ...
        if not isinstance(functions, dict):
            ...
            for func_name, func_config in functions.items():
                if not isinstance(func_config, dict):
                    errors.append(
                        f"Function '{func_name}' config must be a dictionary"
                    )
                    continue
                ...
                if 'enabled' not in func_config:
                    errors.append(
                        f"Function '{func_name}' missing 'enabled' field"
                    )

                if 'kwargs' not in func_config:
                    errors.append(
                        f"Function '{func_name}' missing 'kwargs' field"
                    )
                elif not isinstance(func_config['kwargs'], dict):
                    errors.append(
                        f"Function '{func_name}' kwargs must be a dictionary"
                    )

        return errors

    def _validate_against_template(
        self, config: dict[str, Any], extends_path: str, module_name: str
    ) -> list[str]:
        ...
        if template_module != module_name:
            errors.append(
                f"Module name mismatch: config uses '{module_name}', template is '{template_module}'"
            )
        ...
        for param_name, param_info in template_params.items():
            if param_info.get('required', False):
                if param_name not in config_kwargs:
                    errors.append(
                        f"Function '{func_name}' missing required parameter: {param_name}"
                    )
                elif isinstance(
                    config_kwargs[param_name], str
                ) and config_kwargs[param_name].startswith('#'):
                    errors.append(
                        f"Function '{func_name}' parameter '{param_name}' is not configured (still has comment)"
                    )

        # Check for unknown parameters
        for param_name in config_kwargs:
            if param_name not in template_params and not isinstance(
                config_kwargs[param_name], str
            ):
                # Allow comment parameters (starting with #)
                if not (
                    isinstance(config_kwargs[param_name], str)
                    and config_kwargs[param_name].startswith('#')
                ):
                    errors.append(
                        f"Function '{func_name}' has unknown parameter: {param_name}"
                    )

        return errors

    def _validate_against_pypath(
        self, config: dict[str, Any], module_name: str
    ) -> list[str]:
        ...
        for func_name in functions:
            ...
            if not method_info:
                errors.append(
                    f"Function '{func_name}' not found in pypath module '{module_name}'"
                )
    ...
    def validate_database(
        ...
        """
        self.logger.debug(
            f'Validating all modules for database: {database_name}'
        )
        ...
        for config_file in db_dir.glob('*.yaml'):
            ...
            is_valid, errors = self.validate_database_module(
        ...
        self.logger.debug(
            f'Database {database_name}: {passed}/{total} modules passed validation'
        )
