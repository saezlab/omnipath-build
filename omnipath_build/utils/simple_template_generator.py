...
def generate_pypath_template(resource_name: str) -> Optional[Dict[str, Any]]:
    """
    ...
    if '.' in resource_name:
        ...
        logger.info(f"Generating template for function: {module_name}.{function_name}")
...
def _generate_function_template(module_name: str, function_name: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if not func_info:
            logger.error(f"Could not extract info from function {function_name}")
            return None
        ...
        logger.info(f"Successfully generated template for {module_name}.{function_name}")
        return template
        ...
        logger.error(f"Failed to generate template for {module_name}.{function_name}: {e}")
        return None


def _generate_module_template(module_name: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        logger.info(f"Successfully generated template for {module_name} with {len(functions)} functions")
        return template
...
def _discover_module_functions(module, module_name: str) -> Dict[str, Dict[str, Any]]:
    """
...
def _find_function_module(function_name: str) -> Optional[str]:
    """Find which module contains a function by searching all pypath.inputs modules."""
    import pkgutil
...
def _extract_function_info(func) -> Dict[str, Any]:
    """
...
def _discover_output_fields(func) -> Optional[List[str]]:
    """
    ...
    try:
        ...
        if hasattr(first_record, '_fields'):
            ...
            logger.debug("Function returns list/tuple - no field names available")
            return None
...
def _extract_fields_from_annotations(func) -> Optional[List[str]]:
    """
