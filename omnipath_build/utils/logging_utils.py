...
def log_row_count(
    logger: logging.Logger = None, table_name: str = None
) -> Callable:
...
def log_progress(
    total: int = None, interval: int = 10000, logger: logging.Logger = None
) -> Callable:
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args: Any, **kwargs: Any) -> Any:  # noqa: ANN401
            ...
            if hasattr(result, '__iter__') and not isinstance(
                ...
                def track_progress(items: Any) -> Iterator[Any]:  # noqa: ANN401
                    for item in items:
                        progress['count'] += 1

                        if (
                            progress['count'] - progress['last_logged']
                            >= interval
                        ):
                            if total:
                                ...
                                logger.info(
                                    f'{func.__name__} progress: {progress["count"]:,} items'
                                )
                    ...
                    if progress['count'] > progress['last_logged']:
                        logger.info(
                            f'{func.__name__} completed: {progress["count"]:,} items'
                        )
