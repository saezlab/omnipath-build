...
class PostgresDuckDBConnector:
    ...
    def __init__(self, 
                 ...
                 duck_config: Optional[Dict[str, Any]] = None,
        ...
        logger.info(f"Initialized PostgresDuckDBConnector (DuckDB: {duck_path})")
    ...
    def _create_duckdb_connection(self) -> duckdb.DuckDBPyConnection:
        ...
        try:
            if self.duck_config:
                ...
                logger.debug(f"Created DuckDB connection with config: {self.duck_config}")
            else:
                conn = duckdb.connect(self.duck_path)
                logger.debug(f"Created DuckDB connection with default config")
    ...
    def _setup_postgres_extension(self) -> None:
        ...
        try:
            ...
            raise ConnectionError(f"PostgreSQL extension setup failed: {e}") from e
    ...
    def _build_connection_string(self) -> str:
        ...
        if missing_keys:
            raise ValueError(f"Missing required PostgreSQL config keys: {missing_keys}")
    ...
    def execute(self, sql: str, parameters: Optional[list] = None) -> duckdb.DuckDBPyRelation:
        """
    ...
    def create_schema_if_not_exists(self, schema_name: str, database: str = 'pg') -> None:
        """
        ...
        """
        try:
            self.execute(f"CREATE SCHEMA IF NOT EXISTS {database}.{schema_name}")
            logger.debug(f"Ensured schema exists: {database}.{schema_name}")
            
        except Exception as e:
            logger.error(f"Failed to create schema {database}.{schema_name}: {e}")
            raise
    
    @contextmanager
    def temporary_table(self, table_name: str):
        """
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Failed to drop temporary table {table_name}: {e}")
    ...
    def get_row_count(self, table_name: str, schema: str = None) -> int:
        ...
        try:
            ...
            result = self.execute(f"SELECT COUNT(*) FROM {full_table_name}").fetchone()
