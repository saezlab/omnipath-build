...
class SQLAdapter:
    ...
    def __init__(self, schema_mappings: Optional[Dict[str, str]] = None):
        """
    ...
    def _build_replacement_patterns(self) -> List[tuple]:
        ...
        for schema, pg_schema in self.schema_mappings.items():
            ...
            patterns.extend([
                ...
                (f"CREATE OR REPLACE TABLE {schema}.", f"CREATE OR REPLACE TABLE {pg_schema}."),
                (f"DROP TABLE IF EXISTS {schema}.", f"DROP TABLE IF EXISTS {pg_schema}."),
                (f"DROP TABLE {schema}.", f"DROP TABLE {pg_schema}."),
                (f"ALTER TABLE {schema}.", f"ALTER TABLE {pg_schema}."),
                (f"TRUNCATE TABLE {schema}.", f"TRUNCATE TABLE {pg_schema}."),
            ])
            ...
            # Other contexts
            patterns.extend([
                (f"EXISTS (SELECT * FROM {schema}.", f"EXISTS (SELECT * FROM {pg_schema}."),
    ...
    def adapt_sql(self, sql: str) -> str:
        ...
        if adapted_sql != sql:
            logger.debug(f"SQL adapted with schema prefixes")
    ...
    def validate_sql_structure(self, sql: str) -> bool:
        ...
        for pattern in suspicious_patterns:
            if re.search(pattern, sql_upper):
                logger.warning(f"Potentially suspicious SQL pattern detected: {pattern}")
                return False
...
class SQLExecutionManager:
    ...
    def __init__(self, sql_adapter: SQLAdapter, base_dir: Path):
        """
    ...
    def execute_sql_file(self, filename: str, connection) -> Dict[str, any]:
        """
        ...
        try:
            ...
            if not self.sql_adapter.validate_sql_structure(adapted_sql):
                logger.warning(f"SQL structure validation failed for {filename}")
            ...
            if stats['row_count'] is not None:
                logger.info(f"✓ {filename} completed in {elapsed:.2f}s - {stats['row_count']:,} rows")
            else:
    ...
    def get_execution_summary(self) -> Dict[str, any]:
        ...
        total_time = sum(s['elapsed_time'] for s in self.execution_stats.values())
        total_rows = sum(s['row_count'] for s in self.execution_stats.values() if s['row_count'])
