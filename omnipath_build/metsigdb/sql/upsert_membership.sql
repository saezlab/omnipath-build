...
-- adding one.
--
-- `set_label` is NULL throughout v1. `ontology_terms` is empty, so no set in
-- this build carries a readable name.

INSERT INTO metsigdb_membership (
  resource, set_source_id, metabolite_entity_id,
  metabolite_label, metabolite_entity_type,
  inchikey, smiles, hmdb, pubchem, chebi, kegg,
  set_label, set_type, organism, set_size, set_context,
