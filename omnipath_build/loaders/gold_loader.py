...
class GoldLoader(BaseLoader):
    ...
    def _initialize(self) -> None:
        ...
        if not self.transforms_dir.exists():
            raise GoldLoaderError(f"Gold transforms directory not found: {self.transforms_dir}")
        
        # Initialize SQL adapter and execution manager
        self.sql_adapter = SQLAdapter()
        self.execution_manager = SQLExecutionManager(self.sql_adapter, self.transforms_dir)
        ...
        self.logger.info(f"Gold loader initialized, transforms dir: {self.transforms_dir}")
    
    def _get_ordered_sql_files(self) -> List[str]:
        """Get SQL files ordered by numeric prefix in filename."""
        if not self.transforms_dir.exists():
            raise GoldLoaderError(f"Transforms directory not found: {self.transforms_dir}")
        ...
        ordered_filenames = [filename for _, filename in prefixed_files] + unprefixed_files
    ...
    def execute_all(self) -> Dict[str, Any]:
        ...
        self.logger.info(f"Files executed: {summary['successful']}/{summary['total_files']}")
        self.logger.info(f"Total time: {summary['total_time']:.2f}s")
        if summary['total_rows']:
            self.logger.info(f"Total rows processed: {self.format_row_count(summary['total_rows'])}")
    ...
    def execute_step(self, step: int) -> Dict[str, Any]:
        ...
        if step < 1 or step > len(ordered_files):
            raise ValueError(f"Invalid step {step}. Must be 1-{len(ordered_files)}")
    ...
    def validate_gold_data(self) -> None:
        ...
        for table_name, row_count in stats.items():
            ...
            self.logger.info(f"  Total rows: {self.format_row_count(row_count)}")
            
            # Show sample data
            try:
                sample = self.execute_sql(f"SELECT * FROM pg.gold.{table_name} LIMIT 2").fetchall()
