...
class MetadataLoader(BaseLoader):
    ...
    def __init__(self, database_name: str, db_connector):
        """
    ...
    def _initialize(self) -> None:
        ...
        self.resource_configs_dir = Path("databases") / self.database_name / "resource"
        
        # Validate that the resource configs directory exists
        if not self.resource_configs_dir.exists():
            raise LoaderError(f"Resource configs directory not found: {self.resource_configs_dir}")
        
        self.logger.info(f"Using resource configs structure: {self.resource_configs_dir}")
        
        # Create PostgreSQL schemas
        self._create_postgres_schemas()
        
        self.logger.info(f"Metadata loader initialized, resource configs dir: {self.resource_configs_dir}")
    
    def _create_postgres_schemas(self) -> None:
        """Create required schemas in PostgreSQL."""
        try:
            self.create_schemas(['metadata'])
            self.logger.info(f"Created PostgreSQL metadata schema")
        except Exception as e:
            self.logger.error(f"Failed to create PostgreSQL schemas: {e}")
            raise LoaderError(f"Schema creation failed: {e}") from e
    
    def load(self, validate_only: bool = False, yaml_dir: str = None) -> Dict[str, int]:
        """
        ...
        if yaml_dir:
            ...
            if not override_dir.exists():
                raise LoaderError(f"Resource configs directory not found: {override_dir}")
            
            self.resource_configs_dir = override_dir
            self.logger.info(f"Using resource configs directory override: {self.resource_configs_dir}")
    ...
    def _get_table_columns(self, table_name: str) -> Dict[str, str]:
        ...
        tables_file = Path("databases") / self.database_name / "metadata" / "tables.yaml"
        if not tables_file.exists():
            raise LoaderError(f"Metadata tables definition file not found: {tables_file}")
    ...
    def _build_insert_statement(self, table_name: str, dataframe_name: str) -> str:
    ...
    def _extract_value_for_column(self, column_name: str, config: Dict[str, Any], context: str = "resource") -> Any:
        """
    ...
    def _create_metadata_tables(self) -> None:
        ...
        try:
            ...
            self.execute_sql(f"DROP SCHEMA IF EXISTS pg.metadata CASCADE")
            
            # Create schema for metadata
            self.execute_sql(f"CREATE SCHEMA pg.metadata")
            ...
            self.logger.info("Created PostgreSQL metadata tables from YAML definitions")
    ...
    def _load_resources_and_datasets(self) -> Tuple[int, int]:
        ...
        try:
            ...
            if not resources_df.empty:
                ...
                insert_sql = self._build_insert_statement('resources', 'temp_resources')
                self.execute_sql(insert_sql)
                
                # Cleanup
                self.conn.unregister('temp_resources')
                resources_count = len(resources_df)
                self.logger.info(f"Loaded {resources_count} resources into PostgreSQL")
            else:
            ...
            if not datasets_df.empty:
                ...
                # Build dynamic INSERT statement
                insert_sql = self._build_insert_statement('datasets', 'temp_datasets')
                self.execute_sql(insert_sql)
                
                # Cleanup
                self.conn.unregister('temp_datasets')
                datasets_count = len(datasets_df)
                self.logger.info(f"Loaded {datasets_count} datasets into PostgreSQL")
            else:
    ...
    def _process_resource_files(self) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        for config_file in self.resource_configs_dir.glob("*.yaml"):
            ...
            try:
                ...
                if self._has_unfilled_placeholders(config):
                    self.logger.debug(f"Skipping {config_file.name} - has unfilled placeholders")
                    continue
                
                module_name = config.get('module')
                if not module_name:
                    self.logger.warning(f"Config {config_file.name} missing 'module' field")
                    continue
                
                self.logger.info(f"Processing config: {config_file.name} (module: {module_name})")
                
                # Create resource entry using generic extraction
                resource = {}
                for column_name in resource_columns.keys():
                    value = self._extract_value_for_column(column_name, config, "resource")
                    if value is not None:
                ...
                for func_name, func_config in config.get('functions', {}).items():
                    ...
                    func_context['module'] = module_name  # Add module for resource_id extraction
                    func_context['name'] = func_name      # Add function name for name extraction
                    
                    for column_name in dataset_columns.keys():
                        value = self._extract_value_for_column(column_name, func_context, "dataset")
                        if value is not None:
        ...
        self.logger.info(f"Processed {len(resources_data)} resources and {len(datasets_data)} datasets from resource configs")
    ...
    def validate_metadata(self) -> None:
        ...
        try:
            # Get resource count
            resource_count = self.execute_sql(f"SELECT COUNT(*) FROM pg.metadata.resources").fetchone()[0]
            self.logger.info(f"Resources: {resource_count}")
            
            # Get dataset count
            dataset_count = self.execute_sql(f"SELECT COUNT(*) FROM pg.metadata.datasets").fetchone()[0]
            ...
            for resource in resources:
                ...
                display_parts = []
                for i, col_name in enumerate(resource_columns[:3]):  # Show first 3 columns
                    if i < len(resource):
            ...
            for dataset in datasets:
                # Display first few columns dynamically
                display_parts = []
                for i, col_name in enumerate(dataset_columns[:3]):  # Show first 3 columns
                    if i < len(dataset):
    ...
    def get_resource_count(self) -> int:
        ...
        result = self.execute_sql(f"SELECT COUNT(*) FROM pg.metadata.resources").fetchone()
        return result[0] if result else 0
    
    def get_dataset_count(self) -> int:
        """Get total number of datasets."""
        result = self.execute_sql(f"SELECT COUNT(*) FROM pg.metadata.datasets").fetchone()
        return result[0] if result else 0
