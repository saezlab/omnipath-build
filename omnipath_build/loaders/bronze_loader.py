...
class PyPathBronzeLoader(BaseLoader):
    ...
    def _initialize(self) -> None:
        ...
        if not self.resource_configs_path.exists():
            raise BronzeLoaderError(f"Resource configs directory not found: {self.resource_configs_path}")
        
        # Initialize PyPath adapter
        self.pypath_adapter = PyPathAdapter()
        
        # Initialize bronze writer for PostgreSQL operations
        self.bronze_writer = BronzeWriter(self.db_connector.pg_config, self.conn)
        
        self.logger.info(f"PyPath Bronze Loader initialized")
    ...
    def load(self, 
        ...
        for mod_name in modules:
            try:
                ...
                if validate_only:
                    self.logger.info(f"Validation mode - skipping actual data loading")
                    results[mod_name] = 0
                else:
                    module_results = self.load_module(mod_name, function_name, max_rows, force)
    ...
    def get_configured_modules(self) -> List[str]:
        ...
        try:
            ...
            modules.sort()
            self.logger.info(f"Found {len(modules)} configured modules from YAML files")
            return modules
    ...
    def get_module_functions(self, module_name: str, function_name: Optional[str] = None) -> Dict[str, Dict[str, Any]]:
        ...
        try:
            ...
            if not yaml_file:
                self.logger.warning(f"No YAML config found for module: {module_name}")
                return {}
            ...
            if not config or 'functions' not in config:
                self.logger.warning(f"No functions defined in config for module: {module_name}")
                return {}
            ...
            self.logger.error(f"Could not read module functions for {module_name}: {e}")
            return {}
    ...
    def load_module(self, 
        ...
        functions_to_load = self.get_module_functions(module_name, function_name)
        
        if not functions_to_load:
            self.logger.warning(f"No enabled functions found in module {module_name}")
            return {}
        ...
        for func_name, func_config in functions_to_load.items():
            ...
            try:
                ...
                existing_file = self.get_latest_parquet_file(module_name, func_name)
                if existing_file and not force:
                    self.logger.info(f"Parquet file already exists: {existing_file}")
                    self.logger.info("Skipping download. Use --force to re-download.")
                    
                    # Write sample to PostgreSQL bronze from existing parquet
                    self.bronze_writer.write_to_bronze(
                        module_name, func_name, existing_file, 
                        source_type='parquet'
                    )
                ...
                self.logger.info(f"✅ Loaded {self.format_row_count(row_count)} rows")
        ...
        self.logger.info(f"Module {module_name} total: {self.format_row_count(total_rows)} rows")
        return results
    ...
    def load_function(self, 
                     ...
                     max_rows: Optional[int] = None) -> Tuple[Path, int]:
        ...
        if max_rows:
            ...
            for limit_param in ['limit', 'max_results', 'nrows']:
                method_info = self.pypath_adapter.get_method_info(f"{module_name}.{func_name}")
                if method_info:
                    params = self.pypath_adapter.get_method_parameters(method_info.full_name)
        ...
        self.logger.info(f"Calling {module_name}.{func_name} with kwargs: {kwargs}")
    ...
    def get_latest_parquet_file(self, module_name: str, func_name: str) -> Optional[Path]:
    ...
    def get_parquet_row_count(self, parquet_file: Path) -> int:
        ...
        result = self.execute_sql(f"SELECT COUNT(*) FROM read_parquet('{parquet_file}')").fetchone()
