...
class SilverLoader(BaseLoader):
    ...
    def _load_table_definitions(self) -> Dict[str, Dict[str, str]]:
        ...
        if not self.tables_yaml_path.exists():
            raise SilverLoaderError(f"Table definitions file not found: {self.tables_yaml_path}")
        
        import yaml
        with open(self.tables_yaml_path, 'r') as f:
            return yaml.safe_load(f)
    
    def _initialize(self) -> None:
        """Initialize silver-specific attributes."""
        # Use database-specific bronze data path
        self.bronze_path = Path("databases") / self.database_name / "bronze" / "data"
        
        # Resource configs directory for reading YAML configurations
        self.resource_configs_path = Path("databases") / self.database_name / "resource"
        
        # Load SQL transformation functions
        transforms_path = self.silver_config_path / "transformation_functions.sql"
        ...
        self.logger.info(f"Silver loader initialized, bronze path: {self.bronze_path}")
    
    def _load_transformation_functions(self, transforms_path: Path) -> None:
        """Load SQL transformation functions from file."""
        if not transforms_path.exists():
            self.logger.warning(f"Transformation functions file not found: {transforms_path}")
            return
        ...
        try:
            ...
            self.logger.info(f"Loaded SQL transformation functions from {transforms_path}")
    ...
    def _create_silver_tables(self) -> None:
        ...
        try:
            ...
            for table_name in self.table_definitions.keys():
                self.execute_sql(f"DROP TABLE IF EXISTS pg.silver.{table_name} CASCADE")
    ...
    def load(self, resource_id: Optional[str] = None) -> Dict[str, int]:
        """
    ...
    def load_resource(self, resource_id: str) -> Dict[str, int]:
        ...
        if not datasets:
            self.logger.warning(f"No datasets with data_processing found for {resource_id}")
            return {}
        ...
        for dataset in datasets:
            ...
            if not bronze_file:
                self.logger.warning(f"No bronze parquet file found for {resource_id}/{dataset['name']}")
                continue
            ...
            if not target_table:
                self.logger.warning(f"No target_table specified for {resource_id}/{dataset['name']}")
                continue
            ...
            try:
                ...
                self.logger.error(f"Failed to transform {dataset['name']} to {target_table}: {e}")
    ...
    def get_all_resources(self) -> List[str]:
        ...
        try:
            if not self.resource_configs_path.exists():
                self.logger.warning(f"Resource configs directory not found: {self.resource_configs_path}")
                return []
            ...
            for yaml_file in self.resource_configs_path.glob("*.yaml"):
                try:
                    with open(yaml_file, 'r') as f:
                        ...
                        if config and 'module' in config and 'functions' in config:
                    ...
                    self.logger.warning(f"Could not parse config file {yaml_file}: {e}")
                    continue
    ...
    def get_datasets_with_processing(self, resource_id: str) -> List[Dict[str, Any]]:
                    ...
                    with open(yaml_file, 'r') as f:
                        ...
                        if config and config.get('module') == resource_id:
                            ...
                            for func_name, func_config in functions.items():
                                processing_config = func_config.get('processing')
    ...
    def get_latest_bronze(self, resource_id: str, dataset_name: str) -> Optional[Path]:
    ...
    def _transform_and_load_to_postgres(self,
                                      resource_info: Dict[str, Any],
                                      dataset_name: str,
                                      bronze_file: Path,
                                      target_table: str,
                                      data_processing: Dict[str, Any]) -> int:
        ...
        self.logger.info(f"Transforming {bronze_file} -> pg.silver.{target_table}")
        
        # Check if parquet file exists
        if not bronze_file.exists():
            self.logger.warning(f"Bronze parquet file {bronze_file} does not exist")
            return 0
        ...
        try:
            ...
            self.logger.info(f"Inserted {self.format_row_count(row_count)} rows into pg.silver.{target_table}")
            return row_count
    ...
    def _get_available_columns(self, bronze_file: Path) -> Set[str]:
        ...
        try:
            ...
            self.logger.warning(f"Could not get column names from {bronze_file}: {e}")
            return set()
    
    def _build_column_expression(self, 
                               column_name: str, 
        ...
        for mapping in field_mappings:
            if mapping.get('target') == column_name:
                ...
                if source == '_constant':
                    ...
                    if transform:
                        return self._build_transform_expression(source, transform, mapping)
                    else:
                        return f'"{source}"'
                else:
    ...
    def _build_transform_expression(self, source_field: str, transform_func: str, mapping: Dict[str, Any]) -> str:
        ...
        arg_parts = [f'"{source_field}"']  # First argument is always the source field
            ...
            try:
                ...
                if results:
                    for source, count in results:
                        self.logger.info(f"  {source}: {self.format_row_count(count)} rows")
                else:
                ...
                if sample:
                    ...
                    for row in sample:
                        self.logger.info(f"    {dict(zip(cols, row))}")
