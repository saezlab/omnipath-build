...
Updated for the new silver schema with structured identifiers, members,
...
def build_sources(data_root: Path, output_dir: Path) -> pl.DataFrame:
    ...
    Note: The 'source' field is still a simple string in the new schema,
    so this function works without changes.
    ...
    # Silver files are now directly in source directories (data/source/function_name.parquet)
    ...
    for file_path in parquet_files:
        ...
        # Get source name from the parent directory (data/source/silver_entities.parquet)
        print(f"  {Path(file_path).parent.name}: {len(sources)} unique source(s)")
