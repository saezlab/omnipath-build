...
class DatabaseLifecycleManager:
    ...
    def __init__(self, database_name: str):
        ...
        self.logger.info(f"PostgreSQL connection: {self.pg_config['user']}@{self.pg_config['host']}:{self.pg_config['port']}")
    ...
    def init_database(self, force: bool = False) -> bool:
        ...
        try:
            ...
            if directory_exists and not force:
                self.logger.info(f"Database directory exists, ensuring setup is complete: {self.database_name}")
            ...
            if directory_exists and not force:
                self.logger.info(f"Database {self.database_name} setup verified successfully!")
            else:
                self.logger.info(f"Database {self.database_name} initialized successfully!")
            ...
            if not directory_exists or force:
                ...
                self.logger.info(f"  1. Review and customize table definitions in {self.metadata_path}/tables.yaml")
                self.logger.info(f"  2. Add silver table definitions to {self.silver_path}/tables.yaml") 
                self.logger.info(f"  3. Add transformation functions to {self.silver_path}/transformation_functions.sql")
                self.logger.info(f"  4. Add resources using: python database_manager.py add-resources --database {self.database_name} --resources <resource_list>")
                self.logger.info(f"  5. Run: python database_manager.py load --database {self.database_name}")
    ...
    def load_database(self, layers: Optional[List[str]] = None) -> bool:
        """
        ...
        try:
            if not self.db_base_path.exists():
                self.logger.error(f"Database directory not found: {self.db_base_path}")
                self.logger.error("Run 'init' command first to initialize the database")
                return False
            ...
            self.logger.info(f"Loading database {self.database_name} - layers: {', '.join(layers_to_load)}")
            ...
            for layer in layers_to_load:
                ...
                if not success:
                    self.logger.error(f"Failed to load {layer} layer, stopping pipeline")
                    return False
    ...
    def update_layer(self, layer: str, module: Optional[str] = None) -> bool:
        """
        ...
        """
        try:
            self.logger.info(f"Updating {layer} layer" + (f" (module: {module})" if module else ""))
    ...
    def validate_database(self) -> bool:
        ...
        try:
            ...
            # Validate metadata
            try:
                with MetadataLoader(database_name=self.database_name, db_connector=self.db_connector) as loader:
    ...
    def show_status(self) -> bool:
        ...
        try:
            ...
            self.logger.info(f"  Exists: {'✓' if self.db_base_path.exists() else '✗'}")
            ...
            for subdir, name in [
                ...
                self.logger.info(f"  {name}: {'✓' if exists else '✗'} ({file_count} files)")
            
            # Check PostgreSQL database using the unified connector
            try:
                result = self.db_connector.execute("SELECT current_database()").fetchone()
    ...
    def _create_template_files(self):
        ...
        try:
            ...
            if not metadata_tables_file.exists():
                metadata_content = """# Metadata table definitions for {database_name}
...
""".format(database_name=self.database_name)
                ...
                self.logger.info(f"Created metadata tables template: {metadata_tables_file}")
            
            # Create silver/tables.yaml (empty template)
            silver_tables_file = self.silver_path / "tables.yaml"
            if not silver_tables_file.exists():
                silver_content = """# Silver layer table definitions for {database_name}
...
""".format(database_name=self.database_name)
                ...
                self.logger.info(f"Created silver tables template: {silver_tables_file}")
            
            # Create silver/transformation_functions.sql (empty template)
            silver_functions_file = self.silver_path / "transformation_functions.sql"
            if not silver_functions_file.exists():
                functions_content = """-- Transformation functions for {database_name} silver layer
...
""".format(database_name=self.database_name)
                ...
                self.logger.info(f"Created silver transformation functions template: {silver_functions_file}")
        ...
        try:
            ...
            with conn.cursor() as cursor:
                ...
                if not result:
                    ...
                    self.logger.info(f"Created PostgreSQL database: {self.database_name}")
                else:
                    self.logger.info(f"PostgreSQL database already exists: {self.database_name}")
            
            conn.close()
                    
        except Exception as e:
            self.logger.warning(f"Could not create PostgreSQL database: {e}")
            self.logger.info("You may need to create it manually or ensure PostgreSQL is running")
    
    def _load_layer(self, layer: str, module: Optional[str] = None) -> bool:
        """
        ...
        try:
            if layer == 'metadata':
                with MetadataLoader(database_name=self.database_name, db_connector=self.db_connector) as loader:
                    results = loader.load()
                    self.logger.info(f"Metadata loader completed: {sum(results.values())} items loaded")
                    
            elif layer == 'bronze':
                with PyPathBronzeLoader(database_name=self.database_name, db_connector=self.db_connector) as loader:
                    if module:
                    ...
                    self.logger.info(f"Bronze loader completed: {loader.format_row_count(total_rows)} rows loaded")
                    
            elif layer == 'silver':
                with SilverLoader(database_name=self.database_name, db_connector=self.db_connector) as loader:
                    results = loader.load()
                    total_rows = sum(results.values())
                    self.logger.info(f"Silver loader completed: {loader.format_row_count(total_rows)} rows loaded")
                    
            elif layer == 'gold':
                with GoldLoader(database_name=self.database_name, db_connector=self.db_connector) as loader:
                    results = loader.load()
                    if results.get('total_rows'):
                        self.logger.info(f"Gold loader completed: {loader.format_row_count(results['total_rows'])} rows processed")
                    else:
    ...
    def add_resources(self, resources: List[str]) -> bool:
        ...
        """
        try:
            self.logger.info(f"Adding resources to {self.database_name}: {', '.join(resources)}")
            ...
            metadata_tables_file = self.metadata_path / "tables.yaml"
            if not metadata_tables_file.exists():
                self.logger.error(f"Metadata tables definition not found: {metadata_tables_file}")
                self.logger.error("Please define metadata tables first in metadata/tables.yaml")
                return False
            
            # Load metadata table definitions
            with open(metadata_tables_file, 'r', encoding='utf-8') as f:
                metadata_tables = yaml.safe_load(f)
            
            # Get resource metadata fields (excluding auto-generated fields)
            resource_fields = self._get_insertable_metadata_fields(metadata_tables.get('resources', {}))
            ...
            for resource in resources:
                ...
                module_name = resource.split('.')[0] if '.' in resource else resource
                ...
                if not new_template:
                    self.logger.warning(f"Could not generate template for resource: {resource}")
                    self.logger.warning(f"Make sure '{resource}' is a valid PyPath resource name")
                    continue
                
                # Handle incremental addition to existing files
                if output_file.exists():
                    self.logger.info(f"Adding function to existing module config: {module_name}")
                    enhanced_config = self._merge_with_existing_config(output_file, new_template, resource_fields)
                else:
                    self.logger.info(f"Creating new module config: {module_name}")
                    enhanced_config = self._add_metadata_fields_to_template(new_template, resource_fields)
                
                if enhanced_config:
                    # Save enhanced config
                    self._save_resource_config(enhanced_config, output_file, resource)
                    added_count += 1
            ...
            if added_count > 0:
                self.logger.info(f"Successfully added {added_count} resource configurations")
                self.logger.info(f"Next steps:")
                self.logger.info(f"  1. Edit resource configs in {self.resource_path}/")
                self.logger.info(f"  2. Fill in metadata fields and field mappings")
                self.logger.info(f"  3. Run: python database_manager.py load --database {self.database_name}")
                return True
            else:
    ...
    def _get_insertable_metadata_fields(self, table_def: Dict[str, str]) -> List[str]:
    ...
    def _add_metadata_fields_to_template(self, template: Dict[str, Any], metadata_fields: List[str]) -> Dict[str, Any]:
    ...
    def _merge_with_existing_config(self, existing_file: Path, new_template: Dict[str, Any], resource_fields: List[str]) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            if not existing_config or 'functions' not in existing_config:
                self.logger.error(f"Invalid existing config structure in {existing_file}")
                return None
            ...
            for func_name, func_info in new_template.get('functions', {}).items():
                if func_name in existing_config['functions']:
                    self.logger.warning(f"Function {func_name} already exists in {existing_file}, skipping")
                    continue
            ...
            self.logger.error(f"Failed to merge with existing config {existing_file}: {e}")
            return None

    def _save_resource_config(self, config: Dict[str, Any], output_file: Path, resource_name: str) -> None:
        ...
        with open(output_file, 'w', encoding='utf-8') as f:
            ...
            f.write(f"# REQUIRED: Fill in all question marks (?) before loading\n\n")
...
Examples:
  ...
  python database_manager.py status --database omnipath
    ...
    parser.add_argument("command", 
                       choices=['init', 'add-resources', 'load', 'update', 'validate', 'status'],
                       help="Command to execute")
    
    parser.add_argument("--database", "-d", required=True,
                       help="Database name (e.g., omnipath)")
    ...
    parser.add_argument("--module", 
                       help="Specific module to update (for bronze layer)")
